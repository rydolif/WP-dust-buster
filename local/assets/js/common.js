...
//------------------------------acardeon---------------------------
  ...
  $(".faq__block").first().addClass('faq__active');
  $(".faq__active .block__content").slideDown("slow");

  $(".block__header").on("click", function(){
    if ($(this).parent().hasClass('faq__active')) {
      $(this).parent().removeClass('faq__active');
      $(".block__content").slideUp("slow");
    }
    else {
  ...
  if ($(".about__list").length){
    ...
    $(window).on("scroll load resize", function () {
        ...
        if (w_top + 500 >= e_top || w_height + w_top == d_height || e_height + e_top < w_height) {
            $('.about__item_number').spincrement({
                thousandSeparator: "",
                duration: 2000
            });
            show = false;
